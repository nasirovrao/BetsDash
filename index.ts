...
Deno.serve(async (req: Request) => {
  ...
  // добавленным букмекером. 09.09.2026: поднято с 6 до 12 — прямой отчёт
  // "модель плохо распознаёт букмекера" оказался в первую очередь про то,
  // что у активного пользователя уже 6+ разных букмекеров с референсами,
  // старые (или просто идущие по алфавиту раньше) молча обрезались этим
  // потолком — Object.entries(bookmakerNotes) не гарантирует порядок "самые
  // важные первыми".
  const MAX_BOOKMAKER_REFS = 12;
